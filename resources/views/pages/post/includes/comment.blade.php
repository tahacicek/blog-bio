                ...
                <div class="blog-comment">
                    ...
                    <ul class="comments">
                        <li class="clearfix" id="comment{{ $comment->id }}">
                            <div>
                                <a href=""> <img src="{{ asset($comment->user->avatar) }}" class="avatar"
                                        alt=""></a>
                                <div class="post-comments">
                                    <p class="meta">{{ \Carbon\Carbon::parse($comment->created_at)->diffForHumans() }}
                                        <a href="#">{{ '@' . $comment->user->username }}</a> diyo ki : <i
                                    ...
                                    @if ($comment->user->id == Auth::user()->id)
                                        <button type="button" id="{{ $comment->id }}"
                                            class="comment_trash float-end border-0 btn-outline-dark bg-white"><i
                                                class="fa text-danger fa-trash" aria-hidden="true"></i></button>
                                    @endif
                                    <button type="button" id="{{ $comment->id }}" user="{{ Auth::user()->id }}"
                                        ...
                                        <form id="commentParent" method="POST" onsubmit="return false;">
                                            <div class="input-group">
                                                ...
                                                <input type="text" id="comments"
                                                    class=" form-control-sm form-control form-control-alt "
                                                    name="comment"
                            ...
                            @foreach ($comment->children as $child)
                                <ul class="comments" id="comment{{ $child->id }}">
                                    <li class="clearfix">
                                        ...
                                        <div class="post-comments">
                                            <p class="meta">
                                                {{ \Carbon\Carbon::parse($child->created_at)->diffForHumans() }} <a
                                                    href="#">{{ '@' . $child->user->username }}</a> diyo ki : <i
                                                    class="float-end"><a href="#"><small>Şikayet
                                                            Et</small></a></i></p>
                                            <p>
                                                {!! $child->comment !!}
                                            </p>
                                            @if ($child->user->id == Auth::user()->id)
                                                <button type="button" id="{{ $child->id }}"
                                                    class="comment_trash float-end border-0 btn-outline-dark bg-white"><i
                                                        class="fa text-danger fa-trash" aria-hidden="true"></i></button>
                                            @endif
                                            <button id="{{ $child->id }}" user="{{ Auth::user()->id }}"
                                                post="{{ $child->post_id }}"
                                                class=" comment_dislike float-end border-0 me-1 bg-white"><i
                                                    class="fa comment_dislikes{{ $child->id }} fa-thumbs-down text-secondary"
                                                    aria-hidden="true"></i><span
                                                    class="badge text-dark badge-secondary p-2 dislike_point{{ $comment->id }}">
                                                    @if ($child->commentAction)
                                                        {{ $child->commentAction->where('action', 'dislike')->where('comment_id', $child->id)->count() }}
                                                    @else
                                                        0
                                                    @endif
                                                </span></button>
                                            <button id="{{ $child->id }}" user="{{ Auth::user()->id }}"
                                                ...
                                                class="comment_like float-end border-0 me-1  bg-white"><i
                                                    ...
                                                    aria-hidden="true"></i><span
                                                    class="badge text-dark badge-secondary p-2 like_point{{ $comment->id }}">
                                                    @if ($child->commentAction)
                                                        {{ $child->commentAction->where('action', 'like')->where('comment_id', $child->id)->count() }}
                                                    @else
                                                        0
                                                    @endif
                                                </span></button>
